...
#' @param bias argument to pass to [fuzzySim::selectAbsences()] specifying if/how the selection of unoccupied background points should be biased to incorporate survey effort. Can be TRUE to make selection more likely towards the vicinity of occurrence points; or a SpatRaster of weights (bias layer) with higher values where selection should be proportionally more likely, and zero or NA where points should not be placed. Default FALSE.
...
#' @param test numeric value indicating the proportion of presences to set aside for testing each replicate model (if nreps > 0). The default is 0.2, i.e. 20%.
...
getModels <- function(occs, rasts, region = NULL, nbg = 10000, seed = NULL, bias = FALSE, collin = TRUE, maxcor = 0.75, maxvif = 5, classes = "default", regmult = 1, nreps = 0, test = 0.2, file = NULL, verbosity = 2) {
  ...
  if (!is.null(region)) {
    ...
    if (!(methods::is(region, "SpatVector") || methods::is(region, "SpatExtent"))) {
